...
hl.on("hyprland.start", function()
    ...
    hl.exec_cmd("systemctl --user import-environment WAYLAND_DISPLAY HYPRLAND_INSTANCE_SIGNATURE && systemctl --user restart hypridle")
    ...
    hl.exec_cmd("systemctl --user import-environment WAYLAND_DISPLAY HYPRLAND_INSTANCE_SIGNATURE && systemctl --user restart wayscriber")
    ...
    hl.exec_cmd("[workspace 2 silent] ghostty -e tmux")
    hl.exec_cmd("[workspace 5 silent] steam")
    hl.exec_cmd("[workspace 6 silent] " .. progs.task_manager)

    hl.exec_cmd("obsidian --enable-features=UseOzonePlatform,WaylandWindowDecorations --ozone-platform=wayland --enable-wayland-ime")
